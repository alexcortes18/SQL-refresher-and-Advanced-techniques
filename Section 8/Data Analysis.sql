...
SELECT s1.sales_rep, s1.date, s1.sales
FROM	sales s1
INNER JOIN 
	(SELECT 	sales_rep, MAX(date) AS most_recent_date
	FROM	sales
	GROUP BY	sales_rep) as s2
ON s1.sales_rep = s2.sales_rep AND s1.date = s2.most_recent_date;
...
SELECT 	customer_name, order_date, SUM(price) as total_sale
FROM	pizza_orders
...
SELECT 	dt, price,
        ...
        COALESCE(price,ROUND((SELECT avg(price) FROM sp))) AS updated_price_avg,
        COALESCE(price, LAG(price) OVER(ORDER BY dt)) AS updated_price_prior,
        COALESCE(price, ROUND((LAG(price) OVER(ORDER BY dt) + LEAD(price) OVER(ORDER BY dt))/2)) AS updated_price_smoothing
FROM	sp;
